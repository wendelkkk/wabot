...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558287515844@s.whatsapp.net"] // substitua isso pelo seu número
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '558287515844@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall 558287515844...`)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
                ...
                case 'ninjalogo':
                      ...
                      break
                         case 'play':
              if (!isOwner) return reply(mess.only.playoff)
	          if (!isUser) return reply(mess.only.daftarB)
                reply(mess.wait)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=apivinz`)
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Canção encontrada!!!*\nTítulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE ENVIANDO MUSICA, TMJ É NOIX*`
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('Qual motivo?')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK FML ${tels} NAUM PERTURBE OKEI?*`,
